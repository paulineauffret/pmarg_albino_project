...
#
###################################################################################################
input_dir "~/mantle"              #Input data directory
output_dir <- "~/mantle_de_output"     #Output directory
...
#
###################################################################################################
sampleSheet <- paste(input_dir,"/meta_sample_sheet.info",sep="")                              #SampleSheet. /!\ Need an Id column
colonnes_interet <- c(2,3)  #columns to keep
grep_motif <- ".txt"   #input count files motifs
sample_info_name <- "Sample"
#Color parameters
tb <- "grey20"
an <- "antiquewhite3"
colors <- c(rep(tb,4),rep(an,4))
